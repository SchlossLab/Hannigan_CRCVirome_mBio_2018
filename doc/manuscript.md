...
To evaluate the synergistic capabilities of the bacterial and viral signatures within the model, we built a combinatory model using both bacteria community and virome data. The combination model performed as well as the virome model, suggesting no information was gained by adding the bacterial signatures to the virome model **(Figure \ref{sampleproc})**.

## Performance of model after inclusion of adenoma samples
